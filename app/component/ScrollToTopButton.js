...
const ScrollToTopButton = () => {
  ...
  const handleCall = () => {
    window.location.href = 'tel:+919830449015';
  };

  const handleWhatsApp = () => {
    const phoneNumber = '919830449015'; // without "+" and special characters
  ...
  return (
    <div className="fixed bottom-4 right-4 flex flex-col items-end space-y-3 z-50">
      ...
      <button
        onClick={handleCall}
        className="w-12 h-12 flex items-center justify-center bg-red-700 text-white rounded-full shadow-lg hover:bg-red-600 transition-colors duration-200"
        aria-label="Call"
        type="button"
      >
        <CallIcon />
